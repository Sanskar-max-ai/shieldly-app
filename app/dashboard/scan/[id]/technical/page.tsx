...
export default async function TechnicalBriefPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch white-label branding (agency feature)
  const { data: profileData } = await supabase
    .from('profiles')
    .select('brand_name, brand_logo_url, brand_color, plan')
    .eq('id', user.id)
    .single()

  const branding: BrandingProfile = {
    brand_name: profileData?.plan === 'agency' ? (profileData?.brand_name || null) : null,
    brand_logo_url: profileData?.plan === 'agency' ? (profileData?.brand_logo_url || null) : null,
    brand_color: profileData?.plan === 'agency' ? (profileData?.brand_color || null) : null,
  }

  const accentColor = branding.brand_color || '#0f172a'
  ...
  return (
    <div className="bg-white text-slate-900 min-h-screen p-8 sm:p-12 font-sans selection:bg-slate-200 print:p-0">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Report Header */}
        <header className="border-b-4 pb-8 mb-12 flex justify-between items-start" style={{ borderColor: accentColor }}>
          <div>
            <div className="flex items-center gap-3 mb-4">
              {branding.brand_logo_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={branding.brand_logo_url} alt={branding.brand_name || 'Agency Logo'} className="h-9 max-w-[180px] object-contain" />
              ) : (
                <>
              ...
              {branding.brand_name && (
                <span className="text-xl font-black uppercase tracking-tighter">{branding.brand_name} — Security Audit</span>
              )}
            </div>
                ...
                <div key={i} className="group break-inside-avoid">
                  ...
                  <div className="pl-14">
                    ...
                    <div className="flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-[0.2em] mb-5">
                      <span className="px-2.5 py-1 rounded-sm bg-slate-100 text-slate-600 border border-slate-200">
                      ...
                      {getEvidenceLines(issue.details).length > 0 && (
                        <div className="flex flex-wrap gap-2 items-center">
                          ...
                          {getEvidenceLines(issue.details).slice(0, 3).map((line, index) => (
                            <span key={index} className="px-2 py-1 rounded-sm bg-emerald-50 text-emerald-700 border border-emerald-200 font-mono lowercase tracking-normal">
                              {line}
                            </span>
                          ))}
                        </div>
        ...
        <footer className="border-t border-slate-200 pt-8 flex justify-between items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
           <div>{branding.brand_name ? `${branding.brand_name} Security Report` : 'Generated by Zynth Scan'}</div>
           <div>Report ID: {technicalScan.id}</div>
           <div className="print:hidden">Copyright 2026 {branding.brand_name || 'Zynth Security'}. All rights reserved.</div>
        </footer>
      </div>
